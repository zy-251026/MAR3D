...
class Processor(object):
    def __init__(self, file_name, file_len, skull_strip=False):
        ...
        self.resample_path = './preprocessing/resample/'+file_name[10+file_len:]+'.gz'
        self.n4_bias_path = './preprocessing/n4/'+file_name[10+file_len:]+'.gz'
        self.registration_path = './preprocessing/registration/'+file_name[10+file_len:]+'.gz'
        #self.segmentation_path = './preprocessing/segmentation/'+file_name[10+file_len:-7]
        self.skullstrip_path = './preprocessing/skullstrip/'+file_name[10+file_len:]+'.gz'
        self.matrix_path = './preprocessing/registration/'+file_name[10+file_len:-4]+'.mat'
        self.normalisation_path = './preprocessing/normal/'+file_name[10+file_len:]+'.gz'
...
if __name__ == "__main__":
        dataset_name = ['IXI','OASIS','SALD','DLBS','ADNI1','CORR']
        paths = glob.glob('./dataset/'+dataset_name[4]+'/*.nii')
        exist_paths = glob.glob('./preprocessing/normal/'+dataset_name[4]+'*.nii.gz')
        '''
        new_paths = []
        for i in range(len(paths)):
                for j in range(len(exist_paths)):
                        if paths[i][15:] in exist_paths[j]:
                                break
                if j == len(exist_paths)-1:
                        new_paths.append(paths[i])
        print(len(new_paths))
        '''
        for i in range(5):
            Processor(paths[i], len(dataset_name[4])+1).start()
